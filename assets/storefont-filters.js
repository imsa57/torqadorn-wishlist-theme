...
class CollectionFiltersForm extends HTMLElement {
  ...
  connectedCallback() {
    (this.domNodes = queryDomNodes(this.selectors)),
      this.setData(),
      (this.debouncedOnSubmit = debounce((t) => this.onSubmitHandler(t), 500)),
  ...
  initSorting = () => {
    const { sortingInToolbar: t } = this.domNodes;
    this.enableSorting &&
      t &&
      ((t.selectedIndex = t.querySelector("option[selected]").dataset.index),
  ...
  handleSorting = (t) => {
    const { filtersForm: e, sortingInForm: i } = this.domNodes;
    (i.selectedIndex = Number(t) || 0), e.dispatchEvent(new Event("input"));
  };
  setLoadingTarget = () => {
    const t = window.innerWidth < 768 ? "mobile" : "desktop",
      ...
      i =
        "mobile" === t || "fixed" === this.filtersPosition
          ? {}
          : { overlay: e };
    this.loading = new MinimogLibs.AnimateLoading(document.body, i);
  };
  onSubmitHandler(t) {
    t.preventDefault();
  ...
  renderPage = (t, e = !0) => {
    this.loading.start();
    const i = `${window.location.pathname}?section_id=${this.sectionId}&${t}`;
    (cachedFiltersResult.find(({ url: t }) => t === i)
      ? this.renderSectionFromCache
      : this.renderSectionFromFetch)(i)
      .then(() => {
        MinimogTheme.Collection.init(), this.loading.finish(this.scrollToTop);
      })
      .catch(console.error),
      e && this.updateURLHash(t);
  };
  renderSectionFromFetch = (t) =>
    fetch(t)
      ...
      .then((e) => {
        ...
        //     console.log("hiiScratched");
        //   n.querySelector('#only-clothes').classList.add('active');
        // }
        cachedFiltersResult.push({ url: t, html: n }),
          this.renderProductGrid(n);
      })
      .catch(console.error);
  renderSectionFromCache = async (t) => {
    const e = cachedFiltersResult.find(({ url: e }) => e === t);
    // if(document.getElementById('only-clothes')?.classList?.contains('active')){ 
    //     console.log("hiiCached",e.html);
    //     e.html.querySelector('#only-clothes').classList.add('active');
    //   }
    this.renderProductGrid(e.html);
  };
  renderProductGrid = (t) => {
    const e = t.querySelector(".facest-filters-section");
    this.domNodes.section.replaceWith(e), handleBackgroundImageLazyload();
  };
  updateURLHash(t) {
    history.pushState(
      { searchParams: t },
  ...
  scrollToTop = () => {
    ...
    MinimogTheme.scrollIntoView(t, { align: { topOffset: 80 } });
  };
}
